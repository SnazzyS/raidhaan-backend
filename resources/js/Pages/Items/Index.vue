...
<template>
    <AppLayout title="Items">
        ...
        <Card title="Items" description="Maintain the core catalogue, pricing, and category assignment.">
            <template #actions>
            ...
            <div v-if="filteredItems.length" class="-mx-5 -mb-5 overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200 text-sm text-slate-700">
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        <tr v-for="item in filteredItems" :key="item.id" class="hover:bg-slate-50">
                            ...
                            <td class="px-5 py-4 text-right font-medium text-slate-900">MVR {{ Number(item.price || 0).toLocaleString() }}</td>
                            <td class="px-5 py-4">
                                <div class="flex justify-end gap-2">
                                    <Button variant="ghost" size="sm" @click="openEditModal(item)">Edit</Button>
                                    <Button variant="ghost" size="sm" class="text-rose-600 hover:bg-rose-50 hover:text-rose-700" @click="handleDelete(item)">Delete</Button>
                                </div>
                            </td>
                        </tr>
                    </tbody>
